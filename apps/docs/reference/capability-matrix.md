...
| Muse / Meta OAuth                    | OAuth/Auth File（Device Flow）                 | `meta:window`、`meta:weekly`、reset/周期元数据            | 配额读取，不执行模型请求    |
| Meta API Key                         | `meta-api-key`、优先级、模型和密钥测试         | 请求结果与 Provider 返回信息                             | Provider key test           |
...
完整 Muse / Meta 支持需要 CPA `v7.3.4+`。OAuth DCA 与 `meta-api-key` 是两类独立凭证，不能互换。
